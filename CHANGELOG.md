## 5.3.2
* fix: 본인인증·결제창의 닫기(X) 를 누르면 결제 화면뿐 아니라 이를 호출한 화면까지 닫히던 문제 수정
  - README·예제 안내대로 `onClose` 에서 `Bootpay().dismiss(context)` 를 호출하면 SDK 의 자동 닫기와 겹쳐 두 번 pop 되었다 — 결제 화면이 아직 맨 위일 때만 SDK 가 닫도록 변경
  - iOS 시뮬레이터에서 다날 본인인증 X 로 재현·수정 확인 (수정 전: 메인 메뉴까지 이동 / 수정 후: 본인인증 화면으로 복귀)
* fix: PG 페이지가 최상위 문서에서 `window.close()` 로 닫기를 요청할 때 앱에 아무 이벤트도 전달되지 않던 경로 보완 — `onClose` 로 통지 (Android/iOS 네이티브 SDK · React Native 와 동일 동작)
* fix: `showCloseButton: true` 로 표시한 네이티브 닫기 버튼이 `onCancel` 만 호출하고 창을 닫지 않던 문제 수정 — `onClose` 호출과 함께 화면을 닫는다

## 5.3.1
* fix(ios): 키보드 표시/입력 필드 포커스 이동 시 결제·인증 화면이 회색 박스로 가려지는 문제 수정 (iOS 17.2+, 다크모드 기기에서 두드러짐)
  - 원인: Flutter 의 키보드 리사이즈와 별개로 WebKit 이 자체 키보드 인셋을 비동기 적용해 콘텐츠가 이중으로 밀리며 페이지 아래(under-page) 영역이 노출 (flutter/flutter#140501 계열)
  - `bootpay_webview_flutter_wkwebview` `^3.23.34` 로 상향 — 키보드 노티피케이션 시점에 인셋 재상쇄 + 메인/팝업 WebView 배경(underPageBackgroundColor 포함) 흰색 고정
  - 본인인증(다날 SMS 등) 같이 입력 필드가 여러 개인 짧은 페이지에서 재현되던 증상으로, 시뮬레이터 재현·수정 검증 완료
