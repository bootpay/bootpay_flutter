...
import 'package:bootpay_flutter/user_info.dart';
...
extension BootpayMethod on _BootpayWebViewState {
  Future<List<String>> getBootpayJSBeforeContentLoaded() async {
    ...
    if(this.widget.payload?.extra?.quick_popup == 1 && this.widget.payload?.extra?.popup == 1) {
  ...
  String getBootpayJS() {
    return "BootPay.request(${this.widget.payload.toString()})" + error() + cancel() + ready() + confirm() + close() + done();
  }
  ...
  void transactionConfirm(String data) {
    _controller.future.then((controller) {
      controller.evaluateJavascript("var data = JSON.parse('$data'); BootPay.transactionConfirm(data);");
    });
  }
...
extension BootpayCallback on _BootpayWebViewState {
  ...
  JavascriptChannel onReady(BuildContext context) {
    return JavascriptChannel(
        name: 'BootpayReady',
        onMessageReceived: (JavascriptMessage message) {
          if(this.widget.onReady != null) this.widget.onReady!(message.message);
        });
  }
